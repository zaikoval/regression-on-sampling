...
void GraphTreeNode::setString(int pos, char *s, double f) {
...
void GraphTreeNode::addString(int pos, char *s, double f) {
  if (s[pos]==0) frequency+=f;
  else {
    if (s[pos]=='1') {
      if (one==NULL) one = new GraphTreeNode();
...
void GraphTree::setString(char *s, double f) {
  root->setString(0, s, f);
}

void GraphTree::addString(char *s, double f) {
