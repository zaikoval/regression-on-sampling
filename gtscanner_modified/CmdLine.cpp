...
/******* Graphlets *******/
char CmdLine::orbits_file[MAX_BUF];
char CmdLine::orbits_dir[MAX_BUF];
/*************************/
...
Graph *CmdLine::g = NULL;

FILE *CmdLine::f_output;
FILE *CmdLine::f_occ;
/************ Graphlets ************/
FILE *CmdLine::f_orbits;
/***********************************/
...
/************ Graphlets ************/
bool  Global::compute_orbits;
bool CmdLine::compute_gda;
GDA  CmdLine::gda;
/***********************************/
...
FILE **CmdLine::f_occ_t;
FILE **Global::occ_file_t;

pthread_mutex_t SharedParallel::call_mutex      = PTHREAD_MUTEX_INITIALIZER;
pthread_mutex_t SharedParallel::waiting_mutex   = PTHREAD_MUTEX_INITIALIZER;

pthread_mutex_t SharedParallel::answer_mutex    = PTHREAD_MUTEX_INITIALIZER;
pthread_cond_t SharedParallel::answer_cond      = PTHREAD_COND_INITIALIZER;

bool SharedParallel::no_more_work;
int  SharedParallel::num_nodes;
int  SharedParallel::waiting_threads;
/***********************************/

// Misc Functions
vector<string> &split(const string &s, char delim, vector<string> &elems);
vector<string> split(const string &s, char delim);
vector<string> getFilesInDir(char* dir);
...
void CmdLine::run_gtrie(Graph *g, GraphTree *sg) {
  ...
  if(Global::compute_orbits){ 
	//gt_original->writeGraphletCountToFile(g->numNodes(), f_output);
	computeGDD(gt_original->orbit_frequency, g->numNodes(), gt_original->total_orbits, f_orbits);
    gt_original->freeOrbitFrequencies(g->numNodes());
  }
  /*****************************/
}

/******* Graphlets *******/
void CmdLine::computeGDD(long long int** orbit_frequencies, int num_nodes, int total_orbits, FILE* out){
  for(int i = 0; i < gt_original->total_orbits; i++){
	std::map<int, float> distinct;
    distinct[0] = 0;

    for(int j = 0; j < num_nodes; j++)
      distinct[orbit_frequencies[j][i]]++; 

    gdd.push_back(distinct);

    distinct.clear();
  }
  
  for(int o = 0; o < gdd.size(); o++){
	fprintf(out, "%d-", o);
    
    for(std::map<int,float>::iterator it = gdd[o].begin(); it != gdd[o].end(); ++it){
	  if(it->first != 0)
		fprintf(out, "%d:%.0f,", it->first, it->second);
	}
	fprintf(out, "\n");
  }
}
...
// Run FASE method on graph 'g'
void CmdLine::run_fase(Graph *g, int motif_size, GraphTree *sg) {
  Fase::typeLabel = LSLabeling::TYPE_PICK;
  Fase::census(g, motif_size, sg);
}
...
void CmdLine::prepare_files() {
  /********** Graphlets *********/
  if (Global::compute_orbits){
	vector<string> tokens = split(graph_file, '/');
	string directory = string(orbits_dir).append("/");
	
	struct stat st = {0};
	if (stat(directory.c_str(), &st) == -1) mkdir(directory.c_str(), 0700);
    directory = directory.append(dir?"dir":"undir").append(std::to_string(motif_size).append("/"));
	if (stat(directory.c_str(), &st) == -1) mkdir(directory.c_str(), 0700);
	
	string file = directory.append(tokens[tokens.size()-1]);
	strcpy(orbits_file, file.c_str());
	
	f_orbits = fopen(orbits_file, "w");
    if (f_orbits==NULL)
      Error::msg("Unable to open orbits file \"%s\"", orbits_file);
  }
  /******************************/
  ...
  if (occurrences) {
    ...
    /************* Pthreads ************/
    f_occ_t = (FILE**)malloc(sizeof(FILE*) * Global::num_threads);
    char* file_name = (char *)malloc(strlen(occ_file) + Global::num_threads);

    for(int i = 0; i < Global::num_threads; i++){
      sprintf(file_name, "%s%d", occ_file, i);
      fflush(stdout);
      f_occ_t[i] = fopen(file_name, "w");
      if (f_occ_t[i]==NULL)
        Error::msg("Unable to open occurrences file \"%s\"", file_name);
    }
    free(file_name);
    /**********************************/
  }
...
void CmdLine::compute_original() {
  ...
  /************* Graphlets ***********/
  if(Global::compute_orbits && (method != GTRIE)){
    Error::msg("To compute orbits use \"-m gtrie <gtrie_file>\"");
  } 
  /***********************************/ 
  
  else if (method == SUBGRAPHS) run_subgraphs(g, &sg_original);
  else if (method == GTRIE) run_gtrie(g, &sg_original);
  else if (method == FASE)  run_fase(g, motif_size, &sg_original);
  Timer::stop(0);  
  
  if(method == GTRIE || method == SUBGRAPHS || method == ESU){
	printf("%d subgraphs, ",   sg_original.countGraphs());
    printf("%.0f occurrences\n", sg_original.countOccurrences());
    printf("Time elapsed: %.6fs\n\n", Timer::elapsed(0));
  }
  else if (method == FASE){
	printf("%lld subgraphs, ", Fase::getTypes());
    printf("%lld occurrences\n", (long long int)(Fase::MotifCount));
    printf("Time elapsed: %.6fs\n\n", Timer::elapsed(0));
  }
...
void CmdLine::compute_results() {
  ...
  if (random_number > 0) {
    ...
    mapStringInt *m_count = new mapStringInt[random_number];
  ...
  /******** Graphlets ******/
  if (Global::compute_orbits)
    printf("Orbits (GDD) written to file \"%s\"\n", orbits_file);
  /*************************/
...
void CmdLine::show_results(ResultType *res, int nres) {
  ...
  for (i=0; i<nres; i++) {
    ...
    if (html) 
      fprintf(f_output, "<tr class=\"%s\"><td><img src=\"http://www.dcc.fc.up.pt/gtries/graph.php?%swidth=75&height=75&adj=%s\"></td><td class=\"pre\">%s</td><td>%.0f</td><td>%.2f</td><td>%.2f +/- %.2f</td></tr>\n",
	      (i%2)?"odd":"even", dir?"dir&":"",
	      res[i].s, adj, res[i].f_original,
	      res[i].z_score, res[i].avg_random, res[i].dev_random);
    else
      fprintf(f_output, "%s%*s %lld | %8.2f | %10.2f +/- %10.2f\n\n",
...
void CmdLine::finish() {
  ...
  /**********************************/
    ...
    /*********** Graphlets ************/
    if(Global::compute_orbits) fclose(f_orbits);
    /**********************************/
  }
...
void CmdLine::defaults() {
  ...
  random_tries        = 10;
  /************* Pthreads ************/
  //Global::num_threads = std::thread::hardware_concurrency();
  //if(Global::num_threads == 0)
  Global::num_threads = 1;
  /***********************************/

  /************ Graphlets ************/  
  strcpy(orbits_dir, "orbits");
  Global::compute_orbits = false;
  compute_gda            = false;
  /***********************************/
...
MethodType CmdLine::str_to_method(char *s) {
  ...
  else if (!strcmp(s, "fase"))      return FASE;
...
void CmdLine::parse_cmdargs(int argc, char **argv) {
  ...
  for (int i=1; i<argc; i++) {
    ...
    /************* Pthreads ************/
    // Number of threads
    else if (!strcmp("-th",argv[i]) || !strcmp("--threads",argv[i])) {
      Global::num_threads = atoi(argv[++i]);
    }
    /**********************************/
    /************ Graphlets ***********/
    // Compute orbits?
    else if (!strcmp("-or",argv[i]) || !strcmp("--orbits",argv[i])) {
      Global::compute_orbits = true;
    }
    // Orbits dir
    else if (!strcmp("-odir",argv[i]) || !strcmp("--orbitsdir",argv[i])) {
      strcpy(orbits_dir, argv[++i]);
    }
    // Compute GDA in the given dir
    else if (!strcmp("-gda", argv[i])) {
      compute_gda = strcpy(graph_file, argv[++i]);;
    }
    /**********************************/
  }
...
/********** Graphlets ********/
void CmdLine::compute_gda_dir(){
  vector<string> gdd_files;
  
  if (!strcmp(graph_file, INVALID_FILE))
    Error::msg("Need to specify orbits (GDD) directory with '-g'");
    
  struct stat info;

  if(stat( graph_file, &info ) != 0 )
    Error::msg("Cannot access %s", graph_file );
  else if( info.st_mode & S_IFDIR )  // S_ISDIR() doesn't exist on my windows 
    printf("Using '%s' as the orbit(GDD) directory\n\n", graph_file );
  else
    Error::msg("%s is no directory", graph_file );
    
  gdd_files = getFilesInDir(graph_file);
  vector<string> gddFiles;
  
  string file;
  for(int i = 0; i < gdd_files.size(); i++){
	file = string(graph_file).append("/");
	file = file.append(gdd_files[i].c_str());
	
	gddFiles.push_back(file);
    gda.newGDD(file.c_str());
  }
  
  //float gda_val = gda.computeGDA(0, 1);
  //printf("GDA: %.5f\n", gda_val);
  
  float gda_val = 0;
  for(int i = 0; i < gddFiles.size(); i++){
	for(int j = i+1; j < gddFiles.size(); j++){
	  gda_val = gda.computeGDA(i, j);
	  
	  printf("GDA(%s,%s): %.5f\n", gddFiles[i].c_str(), gddFiles[j].c_str(), gda_val);
	}
  }
}
/*****************************/
...
vector<string> getFilesInDir(char* dir_name){
  vector<string> files;	
  ...
  if ((dir = opendir (dir_name)) != NULL) {
    ...
    closedir (dir);
	  ...
	  Error::msg("Could not open '%s'", dir_name);
  }
